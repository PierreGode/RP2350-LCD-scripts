    ...
    for (size_t i = 0; str[i] != '\0'; i++) {
        ...
        switch(c) {
            ...
            case '$':  // AltGr+4 on Swedish (Shift+4 gives ¤)
                ...
                Keyboard.releaseAll();
                break;
            case '{':  // AltGr+7 on Swedish
                Keyboard.press(KEY_RIGHT_ALT);
                Keyboard.press('7');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case '}':  // AltGr+0 on Swedish
                Keyboard.press(KEY_RIGHT_ALT);
                Keyboard.press('0');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case '[':  // AltGr+8 on Swedish
                Keyboard.press(KEY_RIGHT_ALT);
                Keyboard.press('8');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case ']':  // AltGr+9 on Swedish
                Keyboard.press(KEY_RIGHT_ALT);
                Keyboard.press('9');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case '@':  // AltGr+2 on Swedish
                Keyboard.press(KEY_RIGHT_ALT);
                Keyboard.press('2');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case '\\': // AltGr+Plus key (key right of 0)
                Keyboard.press(KEY_RIGHT_ALT);
                Keyboard.press('+');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case '|':  // AltGr+< (< is on key left of Z, needs raw scancode)
                // The < key is scancode 0x64, but write('<') should work
                Keyboard.press(KEY_RIGHT_ALT);
                Keyboard.write('<');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
            ...
            // Shifted number row (different from US)
            case '=':  // Shift+0 on Swedish (unshifted 0 key gives 0)
                Keyboard.press(KEY_LEFT_SHIFT);
                Keyboard.press('0');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case '(':  // Shift+8 on Swedish
                Keyboard.press(KEY_LEFT_SHIFT);
                Keyboard.press('8');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case ')':  // Shift+9 on Swedish
                Keyboard.press(KEY_LEFT_SHIFT);
                Keyboard.press('9');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case '/':  // Shift+7 on Swedish
                Keyboard.press(KEY_LEFT_SHIFT);
                Keyboard.press('7');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
            ...
            // Other repositioned characters
            case '+':  // Unshifted key right of 0 on Swedish
                Keyboard.write('+');
                break;
            case '-':  // '-' on Swedish is on the key right of '.' (US layout sends '/' for that physical key)
                Keyboard.write('/');
                break;
            case '_':  // Shift + (key right of '.') => '_' (US layout uses Shift+'/' for that physical key)
                Keyboard.press(KEY_LEFT_SHIFT);
                Keyboard.write('/');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case ':':  // Shift+period on Swedish
                Keyboard.press(KEY_LEFT_SHIFT);
                Keyboard.write('.');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
                break;
            case ';':  // Shift+comma on Swedish
                Keyboard.press(KEY_LEFT_SHIFT);
                Keyboard.write(',');
                delay(TYPE_COMBO_DELAY_MS);
                Keyboard.releaseAll();
            ...
            // Standard characters that work the same
            default:
                Keyboard.write(c);
                break;
        }
